...
test1:
	...
	@diff -q test1.txt tests/simplegraphtest3.txt
	@rm test1.txt

test2:
	@./kClistMatrix 1 4 tests/simplegraph.txt 1 > test2.txt
	@diff -q test2.txt tests/simplegraphtest4.txt
	@rm test2.txt

test3:
	@./kClistMatrix 1 5 tests/simplegraph.txt 1 > test3.txt
	@diff -q test3.txt tests/simplegraphtest5.txt
	@rm test3.txt

test4:
	@./kClistMatrix 1 3 tests/edgelist.txt 1 > test4.txt
	@diff -q test4.txt tests/edgelisttest3.txt
	@rm test4.txt

test5:
	@./kClistMatrix 1 4 tests/edgelist.txt 1 > test5.txt
	@diff -q test5.txt tests/edgelisttest4.txt
	@rm test5.txt

test6:
	@./kClistMatrix 1 5 tests/edgelist.txt 1 > test6.txt
	@diff -q test6.txt tests/edgelisttest5.txt
	@rm test6.txt

test7:
	@./kClistMatrix 3 3 tests/edgelist.txt 1 > test7.txt
	@diff -q test7.txt tests/edgelisttest3.txt
	@rm test7.txt
